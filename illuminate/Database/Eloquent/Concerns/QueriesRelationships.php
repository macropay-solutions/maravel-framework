...
trait QueriesRelationships
{
     ...
     *
     * @param \Illuminate\Database\Eloquent\Model|Collection<\Illuminate\Database\Eloquent\Model> $related
    ...
    public function withAggregate($relations, $column, $function = null)
    {
        ...
        foreach ($this->parseWithRelations($relations) as $name => $constraints) {
            ...
            if ($function) {
                ...
                $expression = $function === 'exists' ? $aggregateColumn : sprintf(
                    '%s(%s)',
                    $function,
                    $aggregateColumn
                );
            ...
            $alias ??= Str::snake(
                preg_replace(
                    '/[^[:alnum:][:space:]_]/u',
                    '',
                    "$name $function {$this->getQuery()->getGrammar()->getValue($column)}"
                )
            );
