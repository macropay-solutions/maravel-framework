...
//                return new Response('', 200, ['Allow' => implode(',', $methods)]);
                    return \app(Response::class, ['', 200, ['Allow' => implode(',', $methods)]]);
                },
            ])->bind($request);
        }
    ...
    protected function addToSymfonyRoutesCollection(SymfonyRouteCollection $symfonyRoutes, Route $route)
    {
        ...
        if (!$name) {
            ...
            throw new LogicException(
                "Unable to prepare route [{$route->uri}] for serialization. Another route has already been assigned name [{$name}]."
            );
        }
