        ...
        $values = $this->updateAtomicValues($batchId, function ($batch) use ($jobId) {
            return [
                ...
                'failed_job_ids' => json_encode(
                    array_values(array_diff((array)json_decode($batch->failed_job_ids, true), [$jobId]))
                ),
            ];
        });
    ...
    public function incrementFailedJobs(string $batchId, string $jobId)
    {
        $values = $this->updateAtomicValues($batchId, function ($batch) use ($jobId) {
            return [
                ...
                'failed_job_ids' => json_encode(
                    array_values(array_unique(array_merge((array)json_decode($batch->failed_job_ids, true), [$jobId])))
                ),
            ];
        });
